...
for omn_idxN, dln_n_dpsi in enumerate(omn_arr):
    ...
    for idx, eta in enumerate(eta_arr):
        ...
        stel = Qsc(rc=rc, zs=zs, nfp=nfp, etabar=eta, B0=B0)
        ae_total_arr_n[omn_idxN, idx] = ae_nae(stel, r, lam_res, Delta_psiA, dln_n_dpsi, 0, plot = False)[3]
...
for omT_idxN, dln_T_dpsi in enumerate(omT_arr):
    ...
    for idx, eta in enumerate(eta_arr):
        ...
        stel = Qsc(rc=rc, zs=zs, nfp=nfp, etabar=eta, B0=B0)
        ae_total_arr_T[omT_idxN, idx] = ae_nae(stel, r, lam_res, Delta_psiA, 0, dln_T_dpsi, plot = False)[3]
...
plt.tight_layout()
plt.savefig('qh_eta{}_scan_fig_1.png'.format(-eta), bbox_inches='tight', dpi = 300)
plt.show()
...
ax.grid(alpha = 0.3, linewidth = 0.5)
plt.tight_layout()
plt.savefig('qh_eta{}_scan_fig_2.png'.format(-eta), bbox_inches='tight', dpi = 300)
plt.show()
...
ax.grid(alpha = 0.3, linewidth = 0.5)
plt.tight_layout()
plt.savefig('qh_eta{}_scan_fig_3.png'.format(-eta), bbox_inches='tight', dpi = 300)
plt.show()
