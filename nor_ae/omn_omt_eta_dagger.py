...
try:
    ...
    ae_nor_total_arr = np.load('data_plot/ae_nor_total_arr_omn_omt_{:.4f}_{}_{}.npy'.format(om_min, om_max, N))
    eta_dagger_arr = np.load('data_plot/eta_dagger_arr_omn_omt_r_{:.4f}_{}_{}.npy'.format(om_min, om_max, N))
    ald_computed = True
    print("files found")
...
np.save('data_plot/ae_nor_total_arr_omn_omt_{:.4f}_{}_{}.npy'.format(om_min, om_max, N), ae_nor_total_arr)
np.save('data_plot/eta_dagger_arr_omn_omt_r_{:.4f}_{}_{}.npy'.format(om_min, om_max, N), eta_dagger_arr)
...
cs = axs[0].contourf(omt_mesh, omn_mesh, np.log10(ae_nor_total_arr), 200, cmap = mpl.colormaps['jet'])
...
axs[0].plot((2/3)*omt_arr, omn_arr, color='black', linestyle='--', linewidth=1, alpha = 0.5)
axs[0].set_xlim(omt_arr[0], omt_arr[-1])
axs[0].set_ylim(omn_arr[0], omn_arr[-1])

# eta dagger plot
cs = axs[1].contourf(omt_mesh, omn_mesh, np.log10(-1*eta_dagger_arr), 200, cmap = mpl.colormaps['jet'])
...
fig.savefig('eta_dagger_omn_omt_r_{}_{}_{}_geo.png'.format(r, om_max, N), format='png', dpi=300, bbox_inches='tight')
plt.show()
