...
omn_arr = np.array([0.5, 1, 5, 10])[::-1] 
omt_arr = np.linspace(0, 20, N)
# print(omt_omn_ratios)
# empty array woth shape
ae_nor_total_arr = np.empty((len(omn_arr), N))
...
for idx_omn, omn in enumerate(omn_arr):
    ...
    for idx_omt, omt in enumerate(omt_arr):
        ...
        ae_nor_total_arr[idx_omn, idx_omt] = -1*ae_nor_total_function_eta(eta)
        # how to code if omt/omn is really close to 2/3


    axs.plot(omt_arr, ae_nor_total_arr[idx_omn, :], linewidth = 5, label = r'$\hat{\omega}_{n}$' + ' = {}'.format(omn))
    # calculate id of the omt closest to 2/3
    
    omt = omn*(2/3)

    axs.scatter(omt, -1*ae_nor_total_function_eta(eta), color = 'k', s = 100, zorder = 10)
    #axs.scatter(omt_arr[idx_twothird_omt], ae_nor_total_arr[idx_omn, idx_twothird_omt], color = 'k', s = 100, zorder = 10)


axs.set_xlabel('$\hat{\omega}_T$', fontsize=30)
...
fig.savefig('omn_plots_r_{}_N_{}_a__{}_b__{}_pres.png'.format(r, N, a, b), format='png', dpi=300, bbox_inches='tight')
plt.show()
