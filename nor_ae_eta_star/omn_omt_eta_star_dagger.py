...
cs = axs.contourf(omt_mesh, omn_mesh, np.log10(ae_nor_total_arr), 100, cmap = mpl.colormaps['jet'])
axs.set_xscale('log')
axs.set_yscale('log')
cb = fig.colorbar(cs, ax=axs, shrink=0.9, label = r'$\log_{10}(\hat{A})$')
cb.set_label(r'$\log_{10}(\hat{A})$',fontsize=18)
# labels
axs.set_ylabel('omn', fontsize=18)
axs.set_xlabel('omt', fontsize=18)
axs.set_title('a = {}, b = {}, nfp = {}, B0 = {} \n'.format(a, b, nfp, B0) + ' ' + r'$r = $' + '{}'.format(r) + ', ' + \
    r'$\bar{\eta} = \bar{\eta}_{*} = $' + '{:.4f}'.format(eta_star), fontsize=16)

# 2/3 line eta = omt/omn < 2/3

axs.plot((2/3)*omt_arr, omn_arr, color='black', linestyle='--', linewidth=1, alpha = 0.5)
axs.text((2/3)*0.5 + 0.2, 0.5, r'$\frac{omn}{omt} = \frac{2}{3}$', fontsize=18, color='black', transform=axs.transAxes)

#axs.text((2/3)*omt_arr[N//2], omn_arr[N//2], r'$\frac{\omega_{t}}{\omega_{n}} = \frac{2}{3}$', fontsize=18, color='black', transform=axs.transAxes)
...
fig.savefig('etastar_omn_omt_r_{}_{}_{}_{}.png'.format(r, N, a, b), format='png', dpi=300, bbox_inches='tight')
plt.show()
