...
angles = np.linspace(0.1, np.pi/2 - 0.1, N)
# omp_arr = np.array([0.5, 1, 2, 3, 4, 5])[::-1] 
omp_arr = np.array([0.5, 1, 5, 10])[::-1] 
omt_pre = (np.cos(angles))**2 # *omp
omn_pre = (np.sin(angles))**2 # *omp

omt_omn_ratios = omt_pre/omn_pre
# print(omt_omn_ratios)
# empty array woth shape
ae_nor_total_arr = np.empty((len(omp_arr), len(angles)))
...
for idx_omp, omp in enumerate(omp_arr):

    for idx_angles, _ in enumerate(angles):

        omn = omp*omn_pre[idx_angles]
        omt = omp*omt_pre[idx_angles]
        # print(omn + omt)
        eta = [eta_star]

        ae_nor_total_arr[idx_omp, idx_angles] = -1*ae_nor_total_function_eta(eta)


    axs.plot(omt_omn_ratios, ae_nor_total_arr[idx_omp, :], linewidth = 4, label = r'$\hat{\omega}_{p}$' + ' = {}'.format(omp))
    #axs.scatter(omp*(np.cos(two_thirds))**2, omp*(np.sin(two_thirds))**2, alpha=1, label = 'omp = {}'.format(omp))


axs.set_xlabel('$\eta = \hat{\omega}_T/\hat{\omega}_n$', fontsize=28)
axs.set_ylabel('$\hat{A}$', fontsize=28)
...
axs.text(0.73, 0.03, 'r  = {}'.format(r), transform=axs.transAxes, fontsize=24)

axs.text(0.05, 0.03, r'$\bar{\eta}_{*}$' + ' = {:.2f}'.format(eta_star) , transform=axs.transAxes, fontsize=24)


# adjust font size of legend

# eta = -0.4
# axs.legend(fontsize=20, loc = 'center left', bbox_to_anchor=(0.02, 0.64), ncols = 2)

# # eta = -0.82
axs.legend(fontsize=20, loc = 'center left', bbox_to_anchor=(0.02, 0.62), ncols = 2)

# # eta = -2.5
# axs.legend(fontsize=20, loc = 'center left', bbox_to_anchor=(0.02, 0.60), ncols = 2)

#axs.set_title(r'$\bar{\eta}_{*}$' + ' = {:.3f}'.format(eta_star) + ', r = {}'.format(r), fontsize=20)

axs.grid(ls="--", alpha = 0.5)
fig.savefig('THESIS_omp_plots_r_{}_N_{}_a__{}_b__{}_{:.2f}_another.png'.format(r, N, a, b, eta_star), format='png', dpi=300, bbox_inches='tight')
plt.show()
