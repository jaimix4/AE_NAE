...
# for region 1 and 2
b_1 = lambda a : -0.04 + ((b_0 - (-0.04))/(0.08 - a_0)) * a

# for region 1 and 2
b_2 = lambda a : -0.015 + ((0.017 - (-0.017))/(a_2 - a_0)) * a
...
if ald_computed == False:
    ...
    for idx, a in enumerate(a_arr):
        ...
        for idx_b, b in enumerate(b_arr):
            ...
            #     eta_arr[idx_b, idx] = np.nan

            #     ae_total_arr[idx_b, idx], ae_nor_total_arr[idx_b, idx] = np.nan, np.nan
            ...
            #     alpha_tilde_arr[idx_b, idx] = np.nan

            #     iota_arr[idx_b, idx] = np.nan
...
# b_3_3 = lambda a: 0.035*np.ones_like(a)

# # for region 4

# b_1_4 = lambda a : -0.04 + ((0.017 - (-0.017))/(a_2 - a_0)) * a

# b_2_4 = lambda a : -0.015 + ((0.017 - (-0.017))/(a_2 - a_0)) * a
...
# axs0.plot(a_arr, b_1_3(a_arr), color = 'black', linestyle = '--', linewidth = 1)
# axs0.plot(a_arr, b_2_3(a_arr), color = 'black', linestyle = '--', linewidth = 1)
# axs0.plot(a_arr, b_3_3(a_arr), color = 'black', linestyle = '--', linewidth = 1)

# axs0.plot(a_arr, b_1_4(a_arr), color = 'black', linestyle = '--', linewidth = 1)
# axs0.plot(a_arr, b_2_4(a_arr), color = 'black', linestyle = '--', linewidth = 1)
...
fig.savefig('qs_ab_space_nfp{}_{}_nor_eta_star_lines.png'.format(nfp, N), format='png', dpi=300, bbox_inches='tight')
plt.show()
