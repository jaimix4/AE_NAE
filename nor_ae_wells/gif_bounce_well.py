...
norm = mpl.colors.Normalize(vmin=np.min(ae_per_lam_nor_arr[:, 0, 0]), vmax=np.max(ae_per_lam_nor_arr[:, 0, 0]))
...
cbar.ax.set_title(label = r'$\hat{A}_{\lambda}/\hat{A}$', fontsize=16)
...
def update_linechart(i):
  ...
  for j in range(i+1):
      ...
      norm = plt.Normalize()
      colors = plt.cm.plasma(norm(ae_per_lam_nor_arr[:, j, 0]))
