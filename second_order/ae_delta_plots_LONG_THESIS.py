...
fig = plt.figure(figsize=(21/1.5, 3*7/1.5))
    #grid = gridspec.GridSpec(1, 3, wspace=0.3)

grid = gridspec.GridSpec(4, 3, height_ratios=[1, 1, 1, 0.05], wspace=0.3, hspace=0.33, width_ratios=[1, 1, 1])
...
for nfp in nfp_arr:
    ...
    if r == 0.0001:
        ...
        levels = np.linspace(1.88e-2, 8.5e-2, 200)
    elif r == 0.001:
        levels = np.linspace(0.15e-2, 1.7e-2, 200)
    ...
    # add text to plot
    ax1.text(-0.13, 1.04, r'$N_{\rm fp}$' + ' = {}'.format(nfp), fontsize = 20, transform=ax1.transAxes, color = 'gray')

    if nfp == 2:
        ax1.text(-0.15, 1.15, r'$r$' + ' = {}'.format(r), fontsize = 20, transform=ax1.transAxes, color = 'black')
...
plt.savefig('ae_delta_LONG_r_{}_omn_{}_omt_{}.png'.format(r, omn, omt), dpi = 300, bbox_inches='tight')
plt.show()
