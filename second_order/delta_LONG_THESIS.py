...
fig = plt.figure(figsize=(21/1.5, 3.3*7/1.5))
    #grid = gridspec.GridSpec(1, 3, wspace=0.3)

grid = gridspec.GridSpec(4, 3, height_ratios=[1, 1, 1, 0.05], wspace=0.3, hspace=0.5, width_ratios=[1, 1, 1])
...
for nfp in nfp_arr:
    ...
    ax1.set_title(r'$\delta\hat{A} / r $', fontsize = 24)

    ax1.text(-0.13, 1.07, r'$N_{\rm fp}$' + ' = {}'.format(nfp), fontsize = 24, transform=ax1.transAxes, color = 'gray')

    ax1.text(0.05, 0.87, r'$r = $' + ' {}'.format(r), fontsize = 20, transform=ax1.transAxes)
    ...
    #     cax = fig.add_subplot(grid[3, 2])  # Colorbar subplot for plots 1 and 2
    ...
    #ax2.set_title(r'$\delta\hat{A} / r, \quad r = $' + ' {}'.format(r), fontsize = 24)

    ax2.set_title(r'$\delta\hat{A} / r $', fontsize = 24)

    ax2.text(0.05, 0.87, r'$r = $' + ' {}'.format(r), fontsize = 20, transform=ax2.transAxes)
    ...
    #ax3.set_title(r'$\delta\hat{A} / r, \quad r = $' + ' {}'.format(r), fontsize = 24)
    #plt.title('(AE2nd / AE1st - 1) ,  at r = {}, omn = {}, omt = {}'.format(r, omn, omt))
    #plt.colorbar()

    ax3.set_title(r'$\delta\hat{A} / r $', fontsize = 24)

    #ax3.text(0.6, 0.85, r'$r = $' + ' {}'.format(r), fontsize = 22, transform=ax1.transAxes)

    ax3.text(0.05, 0.87, r'$r = $' + ' {}'.format(r), fontsize = 20, transform=ax3.transAxes)
    ...
    if nfp == 4:
        ...
        cax = fig.add_subplot(grid[3, :])  # Colorbar subplot for plots 1 and 2
        cbar = fig.colorbar(ax3_plot, cax=cax, orientation='horizontal')
...
plt.savefig('Delta_omn_{}_omt_{}.png'.format(nfp, omn, omt), dpi = 300, bbox_inches='tight')
