...
for idx, nfp in enumerate(nfp_arr):
    ...
    a_mesh_things = a_mesh.copy()
    b_mesh_things = b_mesh.copy()

    a_mesh_things[rcrit_arr < 0.03] == np.nan
    b_mesh_things[rcrit_arr < 0.03] == np.nan
    ...
    if nfp == 3:
        ...
        # find the index of a_mesh closest to 0.05
        # a = 0.1265
        # b = 0.0038
        ...
        idx_a = np.nanargmin(np.abs(a_mesh_things[0,:] - a))
        idx_b = np.nanargmin(np.abs(b_mesh_things[:, 0] - b))
        print(idx_a)
        print(idx_b)
        #[0]

        #idx_b = np.argwhere(b_mesh == b)[1]

        # idx_a = np.argmin(np.abs(a_mesh[0,:] - a))

        # idx_b = np.argmin(np.abs(b_mesh[:,0] - b))
        ...
        plt.contourf(a_mesh, b_mesh, np.log10(delB20_arr), 200)
        plt.plot(a_mesh[idx], b_mesh[idx], 'r*')
        stel.plot_boundary(fieldlines=True, r = 0.03)
        stel.plot_axis()
        plt.show()
